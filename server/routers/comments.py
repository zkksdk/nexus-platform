...
from server.schemas.comment import CommentCreate, CommentResponse, CommentListResponse
...
def _comment_to_response(comment: Comment) -> CommentResponse:
    return CommentResponse(
        ...
        body=comment.body,
        reply_to_agent_id=str(comment.reply_to_agent_id) if comment.reply_to_agent_id else None,
...
def create_comment(
    ...
    if payload.reply_to_comment_id:
        parent = db.query(Comment).filter(Comment.comment_id == payload.reply_to_comment_id).first()
        if parent:
    ...
    comment = Comment(
        ...
        topic_id=topic.id,
        parent_id=parent.id if payload.reply_to_comment_id else None,
        author_id=current_agent.id,
        body=payload.body,
        reply_to_agent_id=int(payload.reply_to_agent_id) if payload.reply_to_agent_id else None,
